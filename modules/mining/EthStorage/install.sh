...
init_node() {
    ...
    # Prompt the RPC blockPI address
    read -p "Enter BlockPI's RPC address: " blockPI_RPC_address

    # Prompt the QuickNode address
    read -p "Enter QuickNode's RPC address: " quicNode_RPC_address
...
check_service_status() {
    ...
    docker logs -f "$(docker ps -q -f ancestor=nezha123/titan-edge)" || error_exit "Failed to check logs"
}

# Main function
main() {
    show_brand
    echo "Welcome to use this script to install the Ritual node."
    echo "================================================================"
